...
if ( class_exists( 'WP_Customize_Control' ) ) {
	 ...
	 *
	 */
	class PlotCustomControl extends WP_Customize_Control {
		protected function get_plot_resource_url() {
			return trailingslashit( get_stylesheet_directory_uri() . '/customizer/');
		}
	}
	 ...
	 *
	 */
	 class PlotImageCheckboxCustomControl extends PlotCustomControl {
		/**
		...
		public function enqueue() {
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array(), '1.0', 'all' );
		}
		/**
	 ...
	 *
	 */
	 class PlotTextRadioButtonCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotImageRadioButtonCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotSingleAccordionCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'plot-custom-controls-js', $this->get_plot_resource_url() . 'js/customizer.js', array( 'jquery' ), '1.0', true );
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array(), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
	 ...
	 *
	 */
	class PlotSimpleNoticeCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotSliderControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'plot-custom-controls-js', $this->get_plot_resource_url() . 'js/customizer.js', array( 'jquery', 'jquery-ui-core' ), '1.0', true );
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array(), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
	 ...
	 *
	 */
	class PlotToggleSwitchCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotSortableRepeaterCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class Plot_DropdownSelect2CustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'plot-select2-js', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/js/select2.min.js', array( 'jquery' ), '4.0.6', true );
			wp_enqueue_script( 'plot-custom-controls-js', $this->get_plot_resource_url() . 'js/customizer.js', array( 'plot-select2-js' ), '1.0', true );
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array(), '1.1', 'all' );
			wp_enqueue_style( 'plot-select2-css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/css/select2.min.css', array(), '4.0.6', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
	 ...
	 *
	 */
	class PlotDropdownPostsCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotTinyMCECustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 *
	 */
	class PlotGoogleFontSelectCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		...
		/**
		 * Should we just show the minimal font showing?
		 */
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->fontList = $this->plot_getGoogleFonts( 'all' );
			// Decode the default json font value
			$this->fontValues = json_decode( $this->value() );

			// Find the index of our default font within our list of Google fonts
			$this->fontListIndex = $this->plot_getFontIndex( $this->fontList, $this->fontValues->font );
		...
		public function render_content() {
			...
			if( !empty($this->fontList) ) {
				...
				<div class="google_fonts_select_control">
					...
					<div class="customize-control-description"><?php esc_html_e( 'Select your weight and style', 'skyrocket' ) ?></div>
					<div class="weight-style">
						<select class="google-fonts-regularweight-style">
							<?php
								foreach( $this->fontList[$this->fontListIndex]->variants as $key => $value ) {
									echo '<option value="' . $value . '" ' . selected( $this->fontValues->regularweight, $value, false ) . '>' . ucfirst(str_replace('italic',' italic', $value)) . '</option>';
								}
							?>
						</select>
					</div>
					...
					<?php if($this->minimal == false) : ?>
						<div class="customize-control-description">Select your italic weight</div>
						<div class="weight-style">
								...
								<?php
									...
									foreach( $this->fontList[$this->fontListIndex]->variants as $key => $value ) {
										...
										if( strpos( $value, 'italic' ) !== false ) {
											echo '<option value="' . $value . '" ' . selected( $this->fontValues->italicweight, $value, false ) . '>' . ucfirst(str_replace('italic',' italic', $value)) . '</option>';
											$optionCount++;
										}
									}
									if( $optionCount == 0 ) {
						...
						<div class="customize-control-description">Select your bold weight</div>
						<div class="weight-style">
							<select class="google-fonts-boldweight-style">
								<?php
									...
									foreach( $this->fontList[$this->fontListIndex]->variants as $key => $value ) {
										...
										if( strpos( $value, 'italic' ) === false ) {
											echo '<option value="' . $value . '" ' . selected( $this->fontValues->boldweight, $value, false ) . '>' . ucfirst(str_replace('italic',' italic', $value)) . '</option>';
											$optionCount++;
										}
									}
		 ...
		 */
		public function plot_getFontIndex( $haystack, $needle ) {
		 ...
		 */
		public function plot_getGoogleFonts( $count = 30 ) {
			// Google Fonts json generated from https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=YOUR-API-KEY
			$fontFile = $this->get_plot_resource_url() . 'inc/google-fonts-alphabetical.json';
			if ( $this->fontOrderBy === 'popular' ) {
				$fontFile = $this->get_plot_resource_url() . 'inc/google-fonts-popularity.json';
			}
	 ...
	 */
	class PlotCustomizeAlphaColorControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'plot-custom-controls-js', $this->get_plot_resource_url() . 'js/customizer.js', array( 'jquery', 'wp-color-picker' ), '1.0', true );
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array( 'wp-color-picker' ), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
	 ...
	 */
	class PlotAlphaColorControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
		...
		public function enqueue() {
			wp_enqueue_style( 'plot-custom-controls-css', $this->get_plot_resource_url() . 'css/customizer.css', array(), '1.0', 'all' );
			wp_enqueue_script( 'wp-color-picker-alpha', $this->get_plot_resource_url() . 'js/wp-color-picker-alpha.js', array( 'wp-color-picker' ), '1.0', true );
	 ...
	 *
	 */
	class PlotPillCheckboxCustomControl extends PlotCustomControl {
		/**
		 * The type of control being rendered
		 */
	 ...
	 */
	if ( ! function_exists( 'plot_url_sanitization' ) ) {
		function plot_url_sanitization( $input ) {
	 ...
	 */
	if ( ! function_exists( 'plot_switch_sanitization' ) ) {
		function plot_switch_sanitization( $input ) {
			if ( true === $input ) {
				return 1;
			} else {
				return 0;
	 ...
	 * @return integer	Sanitized value
	 */
	if ( ! function_exists( 'plot_radio_sanitization' ) ) {
		function plot_radio_sanitization( $input, $setting ) {
	 ...
	 */
	if ( ! function_exists( 'plot_sanitize_integer' ) ) {
		function plot_sanitize_integer( $input ) {
			return (int) $input;
		}
	}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'plot_text_sanitization' ) ) {
		function plot_text_sanitization( $input ) {
			if ( strpos( $input, ',' ) !== false) {
				$input = explode( ',', $input );
			}
			if( is_array( $input ) ) {
	 ...
	 */
	if ( ! function_exists( 'plot_array_sanitization' ) ) {
		function plot_array_sanitization( $input ) {
			if( is_array( $input ) ) {
				foreach ( $input as $key => $value ) {
					$input[$key] = sanitize_text_field( $value );
				}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'plot_hex_rgba_sanitization' ) ) {
		function plot_hex_rgba_sanitization( $input, $setting ) {
			...
			if ( false === strpos( $input, 'rgba' ) ) {
				...
				$input = 'rgba(' . plot_in_range( $red, 0, 255 ) . ',' . plot_in_range( $green, 0, 255 ) . ',' . plot_in_range( $blue, 0, 255 ) . ',' . plot_in_range( $alpha, 0, 1 ) . ')';
			}
			return $input;
		}
	}
	 ...
	 */
	if ( ! function_exists( 'plot_in_range' ) ) {
		function plot_in_range( $input, $min, $max ){
			if ( $input < $min ) {
				$input = $min;
			}
			if ( $input > $max ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'plot_google_font_sanitization' ) ) {
		function plot_google_font_sanitization( $input ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'plot_date_time_sanitization' ) ) {
		function plot_date_time_sanitization( $input, $setting ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'plot_range_sanitization' ) ) {
		function plot_range_sanitization( $input, $setting ) {
			...
			return plot_in_range( $number, $min, $max );
		}
	}
